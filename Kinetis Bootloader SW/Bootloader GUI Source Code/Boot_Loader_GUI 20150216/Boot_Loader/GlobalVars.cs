...
namespace Global_Var
{
    ...
    public struct IDENT_DATA
    {
        ...
        public uint erblk;			                                 // erase block size  下位机执行一次擦除命令时擦除的flash大小
        public uint wrblk;			                                 // write block size  下位机执行一次擦除命令时能够写入的最大字节数
        public uint bl_tbl;		                                     // start of boot-loader table (private data and jump table)
        public uint bl_version;                                      // 下位机中Bootloader版本 bootloader version
        public bool bl_rcs;                                          // reading support
        public ulong addr_limit;                                     //下位机flash空间大小     flashsize
        public ulong boot_addr_start;                                //下位机中Bootloader占用的区域  bootloader area
        public ulong boot_addr_end;
        public ulong relocated_vectors;                              //中断向量重定位地址      relocated vector address
        //public ulong verify_addr_limit;
        public ulong dontcare_addrl;                                 // when do verification, do not compare the contents from 用于校准的flash区间一般为0x3FC~0x3FF，具体请参考芯片RM
        public ulong dontcare_addrh;                                 // dontcare_addrl to dontcare_addrh
        public static ulong binaddress;

    }

    public struct BOARD_MEM                                          //image文件数据缓存区
    {
    ...
    public static class MyVar                                       //常量定义，全局变量实例化
    {
